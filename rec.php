...
$baseURL = 'http://localhost/project/uploads/';
...
if (isset($file) && $file['error'] == 0) {
    ...
    if ($stmt->num_rows > 0) {
        ...
        $update_stmt = $conn->prepare("UPDATE letters SET date = ?, letter_by = ?, receipet_no = ?, dealt_by = ?, issued_file = ?, midway_activity = ?, final_reply = ?, status = ? WHERE letter_no = ?");
        if ($update_stmt) {
            $update_stmt->bind_param("sssssssss", $date, $letter_by, $receipet_no, $dealt_by, $issued_file, $midway_activity, $final_reply, $status, $letter_no);
        ...
        $insert_stmt = $conn->prepare("INSERT INTO letters (date, letter_no, letter_by, receipet_no, dealt_by, issued_file, midway_activity, final_reply, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        if ($insert_stmt) {
            ...
            $insert_stmt->bind_param("sssssssss", $date, $letter_no, $letter_by, $receipet_no, $dealt_by, $issued_file, $midway_activity, $final_reply, $status);
...
<body>
    <div class="form-container">
        ...
        <form action="" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="receipet_no">Receipet No</label>
                <input type="text" id="receipet_no" name="receipet_no" required>
            </div>
            <div class="form-group">
